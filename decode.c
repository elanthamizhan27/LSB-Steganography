...
int flag=0,num=0,l;
char ch;
char str_buffer[sizeof(MAGIC_STRING)+1];
...
Status read_and_validate_decode_args(char *argv[], DecodeInfo *decInfo) //function definition for validate the args
{
	...
	if(argv[3] != NULL)
	{
		if((strstr(argv[3],".txt") != NULL) || (strstr(argv[3],".sh") != NULL)||(strstr(argv[3],".c") != NULL) || (strstr(argv[3],".h") != NULL))  							//check if .txt 
		{
			decInfo->decode_fname = argv[3];    						//assigning the rd position to the decoded file
		}
		else
		{   
			printf("INFO : Output file must be of type '.txt' or '.sh' or '.c' or '.h'\n");
			return e_failure;
		}   
	}
	else
				...
				if(decode_secret_file_extn(decInfo) == e_success)
				{
					...
					if(decode_size(decInfo -> fptr_stego_image) == e_success)
					{
						...
						if(decode_secret_file_data(decInfo) == e_success)
						{
							...
							if(dec_close_files(decInfo)== e_success)
							{
								printf("INFO: Done\n");
							}
							else
							{
								printf("INFO: Closing files is failure\n");
								return e_failure;
							}
						}
						else
...
Status decode_secret_file_extn(DecodeInfo *decInfo) 				//function definition for encoding the secret file extn
{
	...
	if(decode_data_from_image(decInfo -> secret_file_extn_size, decInfo -> fptr_stego_image, decInfo -> fptr_decoded) == e_success)
